...
def starting_state():
    ...
    if random.random() > 0.3:
        future_event_list.append({'Event Type': 'Call Start', 'Event Time': 0, 'User': [1, 'Normal', 'Amateur', 0]})
    else:
...
def data_queue_calculater(data: dict, state: dict, clock: float, name: str, temp = 0):
    ...
    """
    data['Cumulative Stats']['Area Under Queue Length Curve']['{} Queue'.format(name)] += (state['{} Queue'.format(name)] - temp) \
    ...
    data['Maximum Queue Length']['{} Queue'.format(name)] = max(data['Maximum Queue Length']['{} Queue'.format(name)],
                                                                (state['{} Queue'.format(name)] - temp + 1))


def data_queue_user(data: dict, clock: str, name: str, user=None, technically='No', has_CallBack='No'):
    parameter = {"No": 1, "Yes": 3}  # to store clock in right position

    if clock != 'Exit':
        first_user_in_queue = min(data['Queue Users']['{} Queue'.format(name)],
                                      key=data['Queue Users']['{} Queue'.format(name)].get)
        potential = data['Queue Users']['{} Queue'.format(name)][first_user_in_queue][1]
        data['Users'][first_user_in_queue][parameter[technically]] = clock
        data['Queue Users']['{} Queue'.format(name)].pop(first_user_in_queue, None)

        if has_CallBack == 'Yes':
            data['Users'][first_user_in_queue][6] = "it_is_CallBack"
...
def call_start(future_event_list: list, state: dict, clock: float, data: dict, user: list):
    ...
    """
    data['Users'][user[0]] = [clock,       -1,           -1,         None,     None,     user[1],      None,        None]  # -1 means that the user did not serve
    ...
    if user[1] == 'Normal':  # if a normal user call ...
        if state['Amateur Server Status'] == 3:  # if all amateur server are busy ...
            if state['Expert Server Status'] < 2:  # if at least one expert server is free ...
                ...
                if Temp == "CallBack":  # If user would like to use Call-back option
                    data_queue_calculater(data, state, clock, 'Normal CallBack', temp=1)

                elif Temp == "No_CallBack":  # If user do not want to use Call-back option
                    data_queue_calculater(data, state, clock, 'Normal', temp=1)
        ...
        if state['Expert Server Status'] < 2:  # if at least one expert server is free ...
            ...
            if Temp == "CallBack":  # If user would like to use Call-back option
                data_queue_calculater(data, state, clock, 'Special CallBack', temp=1)

            elif Temp == "No_CallBack":  # If user do not want to use Call-back option
                data_queue_calculater(data, state, clock, 'Special', temp=1)
...
def call_end(future_event_list: list, state: dict, clock: float, data: dict, user: list):
    ...
    data['Users'][user[0]][7] = user[2]  # here we store user's service type in user's dictionary
    ...
    if user[2] == 'Expert':  # if the server that want to end his/her last call is expert ...
        if state['Special Queue'] > 0:  # whether the special queue is empty or not ...
            ...
            state['Special Queue'] -= 1
            first_user_in_queue, potential = data_queue_user(data, clock, 'Special')
            fel_maker(future_event_list, 'Call End', clock, state, [first_user_in_queue, 'Special', 'Expert', potential])
            ...
            if state['Normal Queue'] == 0:  # if normal user's queue is empty ...
                if (state['Shift Status'] == 2) or (
                    ...
                    if state['Special CallBack Queue'] > 0:  # if special user's call-back queue is not empty ...
                        ...
                        state['Special CallBack Queue'] -= 1
                        first_user_in_queue, potential = data_queue_user(data, clock, 'Special CallBack', has_CallBack= "Yes")
                        fel_maker(future_event_list, 'Call End', clock, state, [first_user_in_queue, "Special", 'Expert', potential])
                        ...
                        if state['Normal CallBack Queue'] > 0:  # whether normal user's call-back queue is not empty ...
                            ...
                            state['Normal CallBack Queue'] -= 1
                            first_user_in_queue, potential = data_queue_user(data, clock, 'Normal CallBack', has_CallBack= "Yes")
                            fel_maker(future_event_list, 'Call End', clock, state,
                                      [first_user_in_queue, "Normal", 'Expert', potential])
                ...
                state['Normal Queue'] -= 1
                first_user_in_queue, potential = data_queue_user(data, clock, 'Normal')
                fel_maker(future_event_list, 'Call End', clock, state,
                          [first_user_in_queue, 'Normal', 'Expert', potential])
        ...
        if state['Normal Queue'] > 0:  # if normal user's queue is not empty ...
            ...
            state['Normal Queue'] -= 1
            first_user_in_queue, potential = data_queue_user(data, clock, 'Normal')
            fel_maker(future_event_list, 'Call End', clock, state, [first_user_in_queue, 'Normal', 'Amateur', potential])
            ...
            if state['Shift Status'] == 2 or state['Shift Status'] == 3:  # if we are in 2nd or 3rd shift of a day
                if state['Normal CallBack Queue'] > 0:  # if special user's call-back queue is not empty at the moment ...
                    ...
                    state['Normal CallBack Queue'] -= 1
                    first_user_in_queue, potential = data_queue_user(data, clock, 'Normal CallBack', has_CallBack= "Yes")
                    data['Users'][first_user_in_queue][1] = clock
                    fel_maker(future_event_list, 'Call End', clock, state,
                              [first_user_in_queue, "Normal", 'Amateur', potential])
...
def technical_call_end(future_event_list: list, state: dict, clock: float, data: dict, user: list):
    ...
    if state['Special Technical Queue'] == 0:  # whether there is no special user in the technical queue ...
        if state['Normal Technical Queue'] == 0:  # if there is also no normal user in the technical queue ...
            ...
            data_queue_calculater(data, state, clock, 'Normal Technical')
            state['Normal Technical Queue'] -= 1
            first_user_in_queue, potential = data_queue_user(data, clock, 'Normal Technical', technically="Yes")
            fel_maker(future_event_list, 'Technical Call End', clock, state,
                      [first_user_in_queue, "", "", potential])

    else:  # whether there are at least one special user in the technical queue ...
        data['Last Queue Length']['Special Technical Queue'] = state['Special Technical Queue']
        data_queue_calculater(data, state, clock, 'Special Technical')
        state['Special Technical Queue'] -= 1
        first_user_in_queue, potential = data_queue_user(data, clock, 'Special Technical', technically="Yes")
        fel_maker(future_event_list, 'Technical Call End', clock, state, [first_user_in_queue, "", "", potential])
...
def queue_quit(state: dict, user: list, data: dict, clock):
    """
    ...
    while clock < simulation_time:
        ...
        if clock < simulation_time:  # the if block below is ganna call proper event function for that event type
            if current_event['Event Type'] == 'Call Start':
                ...
                queue_quit(state, user, data, clock)
...
def calculate_kpi(data: dict, simulation_time: float) -> dict:
    ...
    for i in data['Cumulative Stats']['Area Under Server Busy time'].keys():
        kpi_results['Server Utilization'][i] = cumulative[i] / (simulation_time * server_number[i])
...
def trace_excel_maker(trace_list: list):
    ...
    columns = list(state.keys())  # list of excel columns headers
    ...
    trace.to_excel('trace_dataframe.xlsx', engine='xlsxwriter')



def calculate_kpi_estimation(replication: int, alpha = 0.05) -> dict:
    """
    Parameters
    ----------
    replication : int
...
kpi_result = calculate_kpi(data, 30 * 24 * 60)
calculate_kpi = calculate_kpi_estimation(10)
